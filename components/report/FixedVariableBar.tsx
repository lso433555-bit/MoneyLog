...
export function FixedVariableBar({ fixedTotal, variableTotal }: { fixedTotal: number; variableTotal: number }) {
  ...
  if (total <= 0) {
    return (
      <section className="ml-card p-4">
        <h2 className="mb-2 text-sm font-semibold text-gray-700 dark:text-gray-300">고정 · 유동 지출 비율</h2>
  ...
  return (
    <section className="ml-card p-4">
      <h2 className="mb-3 text-sm font-semibold text-gray-700 dark:text-gray-300">고정 · 유동 지출 비율</h2>

      <div className="flex h-6 gap-0.5 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800">
        {pctFixed > 0 && <div className="h-full bg-gray-900 dark:bg-gray-400" style={{ width: `${pctFixed}%` }} />}
        {pctVariable > 0 && <div className="h-full bg-coral-500" style={{ width: `${pctVariable}%` }} />}
      </div>

      <div className="mt-3 flex items-center justify-between text-sm">
        <span className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
          <span className="h-2.5 w-2.5 shrink-0 rounded-full bg-gray-900 dark:bg-gray-400" />
          고정 {pctFixed}%
        </span>
